...
**2. Placeholder scan:** Search your plan for any of the patterns from the "No Placeholders" section. Fix them.
...
After completing your own self-review, dispatch a plan-reviewer subagent using `./plan-document-reviewer-prompt.md`:
- `PLAN_FILE_PATH`: path to the plan you just wrote
- `SPEC_FILE_PATH`: path to the spec/design doc from brainstorming

If the reviewer finds issues, fix them inline. Then proceed to user review.

## User Review Gate

After the plan reviewer passes, ask the user to review the written plan:
...
After user approval, offer execution choice:
